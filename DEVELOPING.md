...
**A. Dashboard 보안 (5/8)**
- HTTP Basic Auth middleware ([middleware.ts](src/middleware.ts)) — `DASHBOARD_PASSWORD` env 박혀있을 때만 활성
- Vercel Production env에 `DASHBOARD_PASSWORD=tikcle2026` 박힘 / username default = `tikcle`
- `/api/inngest` + 정적 자산은 우회 (matcher 설정)
- onFailure가 `http_unreachable` + `phase5.computed_at` 있으면 last_error 박지 않음 (Inngest ack false alarm 무시)

**B. SEA TikTok Shop 데이터 검증 (5/8~10)**
- **결론**: 자동 수집 사실상 불가능. lemur 포함 모든 Apify TT Shop actor가 US만 지원 / SEA(Indonesia/Thailand/등) reject
  - pro100chok (현재 Phase 1.5): US only — `Field input.region must be equal to "us"` 에러
  - unseenuser, pratikdani, sentry, lemur, apidojo 모두 시도 → backend "only US available"
- **Apify keyword/hashtag scraper도 한계** — TikTok 자체가 SEA Shop API 막음
  - 검증 actor 3종 (clockworks/apidojo/novi) 모두 hashtag/keyword ~200 결과 cap, Exolyt 3,500개 풀 못 따라잡음
  - Validate 스크립트 보존: [validate-apify-vs-exolyt.ts](scripts/validate-apify-vs-exolyt.ts), [validate-apidojo-vs-exolyt.ts](scripts/validate-apidojo-vs-exolyt.ts), [validate-novi-vs-exolyt.ts](scripts/validate-novi-vs-exolyt.ts)
- **현실적 대안 = Kalodata 수동 입력**:
  - Phase 1.5에서 country!=US tiktok_shop은 자동 skip ([phase1-5-shop.ts](src/lib/inngest/aggregators/phase1-5-shop.ts))
  - 사용자가 Kalodata 화면 복붙 → 내가 SQL INSERT로 products/case_product_sales/influencers 박음
  - Skin1004 ID 케이스 (`08b49329-2069-4e59-acf6-afb8086d61a6`) 검증 완료 — 9 products + 33 influencers 박혀 분석 정상 동작
- 비-US tiktok_shop은 **Phase 3.7 lemur skip + Phase 2 shopCreatorOnly 필터 OFF** ([run-analysis.ts:486-511](src/lib/inngest/functions/run-analysis.ts#L486-L511), [phase3-7-shop-creator.ts:78-90](src/lib/inngest/aggregators/phase3-7-shop-creator.ts#L78-L90)) — lemur가 SEA TT Shop 인덱싱 안 해서 9%만 매칭 → 전체 인플 그대로 분석

**C. clockworks async 패턴 전환 (5/10)**
- `fetchTikTokVideos` ([clockworks-tiktok.ts](src/lib/apify/clockworks-tiktok.ts)) sync → **async (POST runs + poll + dataset fetch)**
- Sync API의 5분 한도 + actor 자체 TIMED-OUT 회피
- `PHASE35_BATCH_SIZE` 200 → **50** (단일 batch 안정성)
- Phase 3.5 / Phase 4b.2 ASR 양쪽에 적용

**D. Phase fix 묶음 (5/6~10)**
- **Phase 3.5 video_id 매칭** ([phase3-5-fans.ts](src/lib/inngest/aggregators/phase3-5-fans.ts)) — clockworks `webVideoUrl`(canonical) vs `contents.url`(변형) 매칭 fail로 fans 0 채워지던 버그. 양쪽 `/(video|photo)/(\d+)` 추출해서 매칭
- **Phase 4b.4 Pass 2/3 max_tokens** ([clusterer.ts](src/lib/anthropic/clusterer.ts)) — 6000/1500 → **16000/4000**. Dr.Reju-all 케이스에서 5,276 토큰 truncation으로 JSON parse 실패하던 패턴
- **Phase 5 언어 화이트리스트** ([phase5-position.ts:23-39](src/lib/inngest/aggregators/phase5-position.ts#L23-L39)) — SWE/LIN/HAU/HNJ 같은 franc false positive → `"확인 불가"` 단일 버킷. 알려진 언어 16개 (en/es/ko/ja/zh/pt/fr/de/th/vi/id/ar/ru/it/tr/ms)
- **startAnalysis auto-force skipped phases** ([upload-actions.ts](src/app/cases/[id]/upload-actions.ts)) — `skipped_reason` 박힌 phase는 글로벌 재실행 시 자동 force_phases에 포함

**E. LATAM 권역 통합 (5/7)**
- 옛 LATAM_ES (MX/AR/CO/CL/PE) + LATAM_BR (BR 단독) → **단일 LATAM** ([countries.ts](src/lib/case-detail/countries.ts))
- Exolyt LATAM export에 BR 자연 포함되므로 분리 운영 무의미
- `case.country = "LATAM"` 권역 통합 케이스 → 시딩 통합 + 매출 6개국(MX/AR/CO/CL/PE/BR) marketplace별 sub (옵션 H 모델 그대로)

**F. TikTok Shop GMV 분포 모듈 (5/5)**
- [ShopCreatorGmvDistribution.tsx](src/components/case-detail/ShopCreatorGmvDistribution.tsx) — case scope의 Shop creator를 lifetime GMV로 5 bucket 분류 ($0 / $1~1K / $1K~10K / $10K~100K / $100K+)
- TT Shop은 케이스별 매출 기여 노출 안 함 → cross-brand lifetime GMV로 "검증된 Shop 셀러" proxy
- case detail Section B (TierDistribution → 분포 → TopGmv 5명) 위치

**G. compare 페이지 개선 (5/6)**
- [CompareCellInteractive.tsx](src/components/case-detail/CompareCellInteractive.tsx) — Hero SKU Top 3 + Meta cluster Top 3 토글 추가
- 클릭하면 SKU별 메가 영상 / cluster별 representative 영상 3개 펼쳐짐
- 옛 "입력 X · 후보 Y" 통계 제거 — 케이스별 무엇이 다른지 노브레이너로 보이게

**H. 인플 CSV 다운로드 (5/8)**
- 케이스 detail 우측 상단 ⬇ 인플 CSV 버튼 ([route.ts](src/app/api/cases/[id]/creators-csv/route.ts))
- brand / handle / follower_count / is_shop_creator / shop_creator_class / lifetime_gmv_usd / video_count / promoted_count / top_views
- UTF-8 BOM 박혀 Excel 한글 OK / 협업 영상 수 desc 정렬

**I. Class A~E 메소드론 룰 (5/5)**
- 영상 수 기반 → **Shop 등록 여부 + promoted 영상 수** ([creator-class.ts](src/lib/case-detail/creator-class.ts))
- A: Shop+promoted≥5 / B: Shop+promoted≥2 / C: Shop+1 / D: non-Shop+promoted≥1 / E: 그 외
- bp-playbook 메소드론 룰과 일치
- lemur 응답 GMV/GPM/post_rate/total_brand_collabs/top_brands/shop_creator_gmv_range 다 파싱 ([lemur-shop-creators.ts](src/lib/apify/lemur-shop-creators.ts))

**J. Top GMV Shop Creator 섹션 (5/5)**
- [TopGmvShopCreators.tsx](src/components/case-detail/TopGmvShopCreators.tsx) — Top 5 GMV 인플 + 각자 top 3 영상
- TikTok Shop case + status=ready 한정 노출

**K. phase37 TTL 30일 캐싱 (5/6)**
- 옛: NULL만 재호출 → 영영 안 갱신
- 새: `tiktok_shop_checked_at < 30일 전`도 재호출 ([phase3-7-shop-creator.ts:14-15](src/lib/inngest/aggregators/phase3-7-shop-creator.ts#L14-L15))
- 다음 케이스에서 인플 풀 겹침 시 자동 절감 + 30일마다 refresh

**L. Backfill 스크립트 (5/5~6)**
- [backfill-shop-creator-gmv.ts](scripts/backfill-shop-creator-gmv.ts) — 옛 phase37로 박힌 Shop creator의 GMV/GPM/range/top_brands lemur로 채우기
- 옵션: `--class-a` (promoted≥5) / `--class-ab` (promoted≥2) / 전체. NOONI 1,852 → 127 / 635 / 2,855 단계 가능
- `.env.local` 직접 로더 박힘 (tsx --env-file 안 먹는 환경 대비)
- `npm run backfill:gmv -- <case_id> [--class-a|--class-ab]`

**M. Next.js 15.5.18 보안 업데이트 (5/8)**
- 12개 advisory (middleware bypass / DoS / SSRF / cache poisoning 등)
- 우리 새 Basic Auth middleware도 영향 가능했음 — 즉시 적용

### 진행 중/완료 케이스 (5/11 기준)

**총 35 케이스 / 26 브랜드**. status=ready 25개.

**US Amazon 21개** (분석 완료):
Anua, Arencia(draft), CKD guaranteed, Dr. Althea, Dr. Reju-all, Dr.Melaxin, Elroel, EQQUALBERRY, heveblue, Kahi, medicube, Mixsoon, Nature Republic, NOONI, Numbuzin, oganacell, SimplyVital, Skin1004, Terez & Honor, Tirtir(running), TOSOWOONG

**US TikTok Shop 9개**: Dr.Reju-all, Dr.Melaxin, Easydew(draft), Kahi, Mixsoon, NOONI, Numbuzin, Skin1004, Terez & Honor

**기타 5개**:
- biodance KR (other)
- Celimax LATAM amazon
- Lefilleo MENA amazon (`e7d4e2ad-6f17-4146-9db8-c3424de9cc0c`)
- Lepique MENA amazon (`26466599-6edb-4ad6-8006-2ef388c8cfa7`)
- **Skin1004 ID tiktok_shop** (`08b49329-2069-4e59-acf6-afb8086d61a6`) — Kalodata 수동 입력 첫 케이스. 9 products + 33 influencers SQL INSERT 박힘. Phase 1.5 자동 skip (비-US) + Phase 3.7 skip + Phase 2 shopCreatorOnly OFF로 정상 동작

**중요 case_id 빠른 참조**:
- NOONI (`d7a4a129-7797-444d-89dc-018fb18154a5`) — TikTok Shop US. backfill 2,711명 채워짐
- Heveblue (`a494d303-d67f-434a-9578-e930a45ea95a`) — brand_keyword/brand_meta_pages SQL UPDATE 박힌 후 분석 OK
- Dr. Reju-all (`0da903a8-ef1b-42b3-b6a2-b5f24ddb4b38`) — brand_keyword/brand_meta_pages SQL UPDATE + Pass 2/3 max_tokens fix 후 정상
- Kahi US Amazon (`29ae3cdb-fb93-4ac2-9c38-e4f02cf94530`) — BSR↔viral 분석 사례 (Hero SKU B0CGX76DV1 median +47%)
- medicube (`11821fa3-9818-41cb-97f8-33c73e108d23`) — Anthropic credit 충전 후 정상
...
### Viral 영상 ↔ BSR 분석 인사이트 (5/11 검증)

12 브랜드 (리쥬올/멜락신/엘로엘/이퀄베리/헤브블루/가히/믹순/네이처리퍼블릭/누니/넘버즈인/스킨천사/테레즈앤어너) Hero SKU 기준 282 events 분석:

**평균 효과**: 1M+ viral 영상 → Hero SKU BSR **median +4.2% / 54% positive** (12 브랜드 통합)

**Halo 지속 곡선** (baseline = upload -14~-7일 평균):
- Day 0-3: +4.7% (즉시 효과 lag)
- **Day 4-7: +7.3% (peak)**
- Day 8-14: +5.6% (점차 감소)
- Day 15-30: -2.2% (baseline 회귀)
- Day 31-60: -14.7% (자연 변동)
- **결론: viral 효과 약 10-14일짜리 단발성**. 지속 매출 = 2주 간격으로 viral 끊임없이 OR 광고 누적

**Sweet spot — BSR 출발 위치별 효과**:
- BSR < 500 (정점): viral 효과 거의 없음 (NOONI 287→+11%, Dr.Melaxin 542→+14%)
- BSR 500-5K: viral 시딩 ROI 최대 (Kahi 391→+47%, Dr. Reju-all 5,183→+25%)
- BSR 5K-10K: 안정적 (+10~20%)
- BSR 50K+: viral 거의 안 움직임 (heveblue 41K→+1.8%) — 광고 우선

**예외 케이스** (viral이 매출 전환 약한 브랜드):
- EQQUALBERRY: 42 events median **-15.5%** / positive 21% — 큰 sample이라 통계 강함
- Numbuzin: 4 events median -10% — sample 작지만 부정 신호
- 가능 이유: entertainment-only 시청자, 가격 미스, hero SKU와 viral 영상 매칭 약함
