...
**관련 운영 룰 (2026-05-04 결정)**:
- 권역 case (LATAM_ES / SEA / MENA) = 권역 안 N개 marketplace 데이터 통합 1개 case
- contents/sales 자식 테이블의 country도 case.country (= 권역 코드)로 통일 박힘
- 국가별 sub-breakdown은 안 됨 (필요해지면 schema 재설계 필요 — phase 코드 fetch 키를 case_id 기반으로 바꿔야 함)
- 권역 case에 1개 marketplace만 업로드해도 기술적으로 작동하지만, "권역 통합" 라벨이 misleading 되니 운영 룰로 N개 다 모아서 업로드
