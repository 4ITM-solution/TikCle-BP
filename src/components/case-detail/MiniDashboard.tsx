...
// =============================================================================
// Module: 티어 분포 (Phase 3)
// =============================================================================
function TierDistributionModule({
  phase3,
  phase35,
  phase37,
}: {
  ...
  // 옛 phase3 데이터에 sub-nano 키 없을 수 있음 → 항상 숫자로 정규화
  const v = (k: keyof typeof dist) => dist[k] ?? 0;
  const max = Math.max(...tiers.map((t) => v(t.key)), 1);
  const total = phase3.total_creators;
    ...
    <div className="section-card">
      ...
      >
        <div>
          ...
          >
            전체 {total.toLocaleString()}명 · fans 있음{" "}
            {phase3.total_with_fans.toLocaleString()} ·{" "}
            unknown {phase3.total_unknown.toLocaleString()}
          </div>
        </div>
        <div
          style={{
      ...
      {phase35 && !phase35.skipped_reason && (
        <div
          style={{
            fontSize: 11,
            color: "var(--color-g500)",
      ...
      {phase37 && !phase37.skipped_reason && (
        <div
          style={{
            fontSize: 11,
            color: "var(--color-g500)",
        ...
        >
          ...
          {" / 미매칭 "}
          <b>{phase37.total_unmatched.toLocaleString()}</b>
          {" · 비용 "}
          <b style={{ color: "var(--color-ink)" }}>
