...
export type LemurShopCreatorItem = {
  ...
  shop_creator_class: string | null; // categories 콤마 join (e.g., "beauty,fashion")
  user_id: string | null; // TikTok user_id (있으면)
  // Performance metrics (Shop creator만 — 응답 stats 안)
  follower_count: number | null;
  lifetime_gmv_usd: number | null; // stats.gmv.total
  gpm_usd: number | null; // stats.gpm
  post_rate: number | null; // stats.postRate
  total_brand_collabs: number | null; // stats.totalBrandCollaborations
  top_brands: unknown[] | null; // stats.topBrands array
  gmv_range: string | null; // e.g., "$5K-$25K"
};
...
function mapItem(
  ...
  // Performance metrics — bp-playbook/case-pipeline/scripts/classify_creators.py spec 그대로
  const followers = stats?.followers as Record<string, unknown> | undefined;
  const gmv = stats?.gmv as Record<string, unknown> | undefined;

  const follower_count = numFrom(followers?.total);
  const lifetime_gmv_usd = numFrom(gmv?.total);
  const gpm_usd = numFrom(stats?.gpm);
  const post_rate = numFrom(stats?.postRate);
  const total_brand_collabs = numFrom(stats?.totalBrandCollaborations);
  const top_brands = Array.isArray(stats?.topBrands)
    ? (stats?.topBrands as unknown[])
    : null;
  const gmv_range =
    typeof stats?.gmvRange === "string"
      ? (stats?.gmvRange as string)
      : typeof gmv?.range === "string"
        ? (gmv?.range as string)
        : null;
