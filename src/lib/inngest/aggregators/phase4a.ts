...
export async function runPhase4a(
  ...
  // 케이스 모두 유효. brand_meta_pages(page_id)가 있으면 수집되고, 없으면 0건 반환.
  // (옛 코드는 amazon만 분석했지만 SharkNinja TT Shop 케이스에서 Meta 광고 분석
  //  필요해서 가드 풀음 — 2026-05-27)
  // ⚠️ 2026-08-10부터 수집은 공식 액터(page_id) 단독. brand_keyword는 수집에 안 쓰이고
  //    classifyLanding()의 DTC 판정에만 쓰인다.
