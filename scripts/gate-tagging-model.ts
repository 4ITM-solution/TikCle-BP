 ...
 * 샘플 30개(영상 20 + 광고 10)를 뽑아 Haiku로 재태깅 → 필드별 일치율 표 출력.
 * DB에 쓰지 않음(읽기 전용). 실행 비용 ~$0.5 이내(하드 캡).
 *
 * 판정: 필드별·종합 일치율 ≥90%면 Haiku 전환 확정 (최종 승인은 오케스트레이터).
 *
 * 사용:
 *   npm run gate:tagging                 # 기본 20영상 + 10광고
 *   npm run gate:tagging -- --videos 30  # 영상만 30개
...
async function main() {
  ...
  const videoN = num("--videos", 20);
  const adN = num("--ads", 10);
  ...
  if (videoN > 0) {
    const { data: rows } = await supabase
      ...
      .limit(videoN * 3); // 여유 조회 후 cover fetch 실패분 보정
    const sample = (rows ?? []).slice(0, videoN * 3);
  ...
  if (adN > 0) {
    const { data: rows } = await supabase
      ...
      .limit(adN * 3);
    const sample = (rows ?? []).slice(0, adN * 3);

    for (const r of sample) {
      if (adCompared >= adN) break;
      if (costUsd >= COST_CAP_USD) {
