...
| BE-10 | ✅ [CX1-F1] startAnalysis 이벤트 발행 실패 성공 위장 fix — inngest.send catch에서 status를 직전 값으로 원복 + last_error='event_dispatch_failed: …' + `ok:false` 반환(직전 status는 update 전에 캡처). tsc 통과. 실패 시뮬 트레이스 WS5_REPORT BE-10 | CX_파이프라인_재감사 F1 | tsc + 실패 시뮬 |
