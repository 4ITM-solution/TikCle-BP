...
| FE-2 | 케이스 상세 **통합 프로토 v9** 확정 시 1:1 구현 (재해석·축소 금지) — v8 개선 13건 + ★교차 뷰 3종(A 이벤트 윈도우·C 성과 탭·B 크리에이터 드로어) + E 원본 후보 + NULL 표기. 정본: docs/design/prototype/bp-case-proto-v9.html + 아티팩트 f30052b0. **선행: Stage 1 조합 데이터 계층(BE 배차 대기 — combo-queries.ts, clusterBundle 추출)** | ⏸ **v11 검수 대기 (2026-07-23 — 2차 피드백 6건: 표준 영상카드 6지표 전면 적용·변곡점 행토글+기준전환·B 요약통합/TT샵·언어 채널종속/50명·C 탭 8→6/토글 재배치·D 채널비교→SKU 우선/TT샵분해 탭·교집합 근거열)** | 플랜 ethereal-meandering-flask + 정보니즈 지도 v2 | 프로토 대비 블록 diff 0 + Kundal 기준값 대사 |
