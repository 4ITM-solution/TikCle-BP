...
| BE-29 | **SKU 채널 간 동일제품 매칭 (사용자 확인 2026-07-26 — 프로토는 수동 픽스처였음)** — Kalodata(한국어·ml) ↔ Amazon(영어·oz) 리스팅을 같은 제품으로 판정: ①이름 토큰+용량 환산 $0 ②카테고리·가격대 보조 ③제품 이미지 대조 ④LLM 쌍 판정(SKU 수십 개뿐 — 전 쌍 ~$0.05) ⑤`products.product_group_id` 영구 저장 + 화면 묶음 해제 UI. D 통합 SKU 표의 실제 백엔드 | ⬜ | 프로토 v11 D 통합 표 + 사용자 지적 | 실케이스(이퀄베리·루미브류 교차 채널)에서 묶음 정확도 사람 검수 통과 |
...
| BE-25 | **크로스채널 동일인 매칭 확장 (G6 구현, 사용자 요청 2026-07-25)** — 현행 norm_handle 완전일치는 채널별 핸들이 다르면 누락. 신호 계단: ①표시이름(full_name) 대조 $0 ②바이오 상호 링크(수집 필드 추가) ③프로필 사진 pHash ④콘텐츠 커버/캡션 크로스포스팅 지문 $0 ⑤후보쌍만 LLM 확정 → `influencers.linked_handles` 저장(1회 판정 영구 재사용). 화면엔 확정/추정 신뢰도 구분 표기 | ⬜ | BP_재설계_v2 §2.1 G6 + 사용자 요청 | 실케이스 1개에서 핸들 상이 동일인 3쌍+ 확정, B 매트릭스 반영 |
