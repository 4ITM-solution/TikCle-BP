...
| FE-2 | 케이스 상세 **통합 프로토 v9** 확정 시 1:1 구현 (재해석·축소 금지) — v8 개선 13건 + ★교차 뷰 3종(A 이벤트 윈도우·C 성과 탭·B 크리에이터 드로어) + E 원본 후보 + NULL 표기. 정본: docs/design/prototype/bp-case-proto-v9.html + 아티팩트 f30052b0. **선행: Stage 1 조합 데이터 계층(BE 배차 대기 — combo-queries.ts, clusterBundle 추출)** | ⏸ **v10 사용자 검수 대기 (2026-07-23 재게시 — 가독성 피드백 6건 반영: 헤더 2줄·콘솔 통합·타임라인 표·B월필터 명단 적용·성과 탭 탐색기 카드 이식·SKU 채널통합+이모지/칩 제거)** | 플랜 ethereal-meandering-flask + 정보니즈 지도 v2 | 프로토 대비 블록 diff 0 + Kundal 기준값 대사 |
