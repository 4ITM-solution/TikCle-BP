...
| # | 작업 | 상태 | 근거 | 완료 기준 |
|---|---|---|---|---|
| FE-3 | 🔴 **케이스 상세 풀 프로토타입** — ①배포판 `/cases/[id]`(page.tsx+case-detail 컴포넌트 전부)를 전수 읽고 **블록 인벤토리 표** 작성(섹션×블록×현재 소스×현재 형태) ②인벤토리의 **전 블록**이 들어간 self-contained HTML 프로토 1장(vanilla JS, 외부 리소스 금지) — 데이터는 파일럿 실측치(docs/ws/QA_파일럿_매트릭스), 토글·탭·정렬·호버 실동작 ③배포판과 다르게 제안하는 블록만 [변경 제안] 칩+기존/제안 병기, FE-1 신규분은 [신규] 칩 ④A 타임라인은 확정안 반영(티어×월 스택 + 광고집행 라인 + BSR 라인 — scratchpad bp-prototype.html v3.1 참고·재사용 가능) ⑤스크립트는 node --check 통과 후 제출. **재해석·축소 금지 — 배포판에 있는 건 다 있어야 함** | ⬜ | docs/design/PROTOTYPE_PROTOCOL.md §1·§3, 사용자 반려 2026-07-11("배포판보다 간소화된 걸 보라는 거냐") | 블록 인벤토리 표 + docs/design/prototype/case-detail-draft.html + 인벤토리 대비 누락 0 자가검증표 | (⭐가동 — U-2 해소 2026-07-08)
