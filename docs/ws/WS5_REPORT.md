...
# CX-1 재감사 후속 (BE-10~12)

## BE-10 — startAnalysis 이벤트 발행 실패 성공 위장 fix ✅ (CX1-F1)

### 원인
`upload-actions.ts` `startAnalysis`: `status='running'`으로 바꾼 뒤 `inngest.send`가 실패하면
catch가 `console.warn`만 하고 그대로 진행 → **(1) 이벤트 미발행이라 오케스트레이터가 영원히 안
돎, (2) status는 running으로 박제, (3) 함수는 `ok:true` 반환** → 화면엔 "분석 시작"인데 아무 일도
안 일어남(성공 위장).

### 수정
- update 전에 `priorStatus` 캡처(`select("status, key_stats")`).
- send 실패 catch에서: `status`를 `priorStatus`로 원복, `key_stats.last_error =
  {message:"event_dispatch_failed: …"}` 기록, `return { ok:false, error }`.

### 실패 시뮬 트레이스 (프로덕션 write 금지 → 코드 경로 추적)
