...
| # | 의도 | 실제 구현 | 대응 (담당) |
|---|---|---|---|
| G1 | 유가 vs 무가 규모 파악 | is_ad=스파크애즈, paid_signal=#ad류 정규식 → **미표기 소액유가는 전부 organic으로 집계** (과소평가) | 라벨을 "표기된 paid"로 정정 + 유가 추정 보조신호(반복 인플·동시다발 업로드 패턴) 연구 (WS4 라벨, 보조신호는 백로그) |
| G2 | "시딩 당시" 티어 구성 타임라인 | **현재 팔로워** 기준 분류 → 같이 큰 크리에이터일수록 상향 왜곡 | "현재 티어 기준" 라벨 + 브리프에 한계 자동 표기 (WS4/WS7) |
