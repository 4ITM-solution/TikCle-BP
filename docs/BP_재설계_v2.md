...
- 처리: §1.0의 케이스 필터(국가×판매채널×예산)로 완결성 높은 매칭 케이스 선택 → "이 케이스대로면 [티어 믹스]·[타임라인]·[소재 앵글]" 운영안 생성.
- 의미: 진단→구독 퍼널의 상품 엔드포인트. WS6(케이스 필터·정규화 축)과 WS7(브리프 생성) 완료가 선행 조건.
...
| WS7 | **Scout 모드**(§1.0.3 — 저비용 훑기 파이프라인 프리셋) + **케이스 브리프 생성기**(§1.4 가공 A — 슬랙/세일즈/미팅 3종) | 신규 브랜드 1개를 Scout으로 <$5·당일 브리프 산출 |
