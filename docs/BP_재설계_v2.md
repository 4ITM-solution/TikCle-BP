...
### 1.2 답해야 할 질문 — 계약 v2 (2026-07-07 개정, CX-2 리뷰 채택)

> v1(Q1~Q7)은 "케이스 안에서 무엇이 일어났나"만 물었다. v2는 케이스를 **고르고(Q0·Q7)·정규화하고(Q1)·조건 검증하고(Q6)·유효기간을 밝히는(Q8)** 질문을 계약에 편입한다. 기존 데이터 투자는 전부 유지 — v1 Q5는 Q3·Q4의 하위 렌즈로, v1 Q6·Q7은 새 Q5로 통합. 근거: `docs/ws/CX_질문계약_리뷰.md`.

| # | 질문 | 역할 | 구현 배정 |
|---|---|---|---|
| Q0 | 이 케이스는 어느 필터(국가×채널×예산/스테이지)에서 채택/보류/폐기인가? | 케이스 후보 게이트 | WS6 (완결성 게이지+core_factor 기반 판정) |
| Q1 | 기간×채널×진입 N개월차 기준으로 얼마나 운영했나? (TT샵 분리 포함) | 규모/시점 | 기존 v1-Q1 + 정규화 축(WS6) |
| Q2 | 누구를 반복적으로 썼고, 티어/채널 구성은? | 구성 | 기존 v1-Q2 (완료) |
| Q3 | 어떤 앵글/USP를 어떤 순서로 썼나? (+성과와 함께 움직인 특징 = 렌즈) | 콘텐츠 | 기존 v1-Q3 (+v1-Q5 렌즈 편입) |
| Q4 | 발행/광고 변화와 매출·BSR·GMV가 언제 함께 움직였나? | 성과 | 기존 v1-Q4 |
| Q5 | 광고는 시딩 자산을 어떻게 재활용했나? (최장 운영 소재·origin·겹치는 인플) | 광고 접합 | v1-Q6+Q7 통합 |
| Q6 | 이 케이스의 최소 성공 조건과 이전 불가능한 조건은? | core_factor | WS6 (LLM 초안→사람 확정 유지) |
| Q7 | 같은 필터 조합의 대표 케이스 top N은 무엇이고 왜 이 순서인가? | L3 매칭 | WS6/WS8 |
| Q8 | 이 답은 얼마나 최신이며 어떤 축은 보류해야 하나? | freshness | 파일럿 D3 주기와 연동 (source별 배지) |

<details><summary>v1 원문 (Q1~Q7, 2026-07-07 최초 확정 — 이력 보존)</summary>
...
- 2026-07-07 (오후): WS1 잔여 검증 **종결** — 스모크(3e2e77be)의 collect-meta 재실행에서 meta_ads 268행 전부 기존 행 유지·ad_intel 263건 보존 확인 (upsert 정상). F1 재개 스크립트 `scripts/f1-resume-waves.sh` 작성(크레딧 충전 후 원클릭: 25케이스 웨이브→serve-stats→legacy 카운트). 실행 세션 워크트리 격리: main 체크아웃=WS3, `.claude/worktrees/ws-4a`=WS4a, `main-orch`=오케스트레이터.
- 2026-07-07 (오후): WS3 코드 완료(브랜치 ws-3-model-tiering, 36a0cdd) — 오케스트레이터 검수 **통과**(tsc 0·dedup 로직·뷰 전제 10,723행 YYYY-MM-DD 확인) + **migration 018 프로덕션 적용·뷰 검증 완료**(tag_input_hash 2컬럼+partial index+v_case_ad_runtime). **머지·배포 보류**: 품질 게이트(`npm run gate:tagging`, 일치율 ≥90%)가 크레딧 충전 대기 — 게이트 통과 후 머지→배포→interpret-tag 1케이스 재실행(reused 확인) 순.
- 2026-07-07 (오후): WS4a rev1 사용자 반려 — U1 과잉적용(채널별 데이터를 전부 "자세히" 뒤로). U1 정정(채널=1급 축, 숨기는 건 측정치×기간×SKU만) + 사용자 목업 v4를 절대 기준으로 등재(docs/ws/ref/) + rev2 지시서(시나리오 3종 전환·v4 정보 밀도 유지·U2~U8은 존치). 교훈: 원칙 문구의 예시 나열이 축 자체 제거로 오독될 수 있음 — 디폴트 기준은 산문이 아니라 레퍼런스 파일로 지정.
- 2026-07-07 (저녁): 화면 기획 트랙 사용자 인수 — WS4a rev2(목업 4종+시나리오 3종, 705a398, ws-4a-screens 브랜치 보존)는 오케스트레이터 검수 통과했으나 사용자가 방향 자체를 직접 잡기로 결정. 화면은 사용자가 별도 진행, 확정본이 오면 WS4b 결선.
- 2026-07-08 (2): BE-10~12 검증·머지·배포 — 발행실패 정직화·fail-closed 가드(emergency cap $5)·**phase cascade DAG(라이브 검증: sku→serve-stats 자동 동반 실측)**. R11(phase 재실행 뒤 serve-stats)이 이제 수동 규칙이 아니라 코드 보장. CX-1 발견 6건 중 5건 당일 종결(F6는 WS8 시점 백로그).
- 2026-07-08: **U-2 해소 — 화면 기획 확정: 리디자인 폐기, 현행 배포 화면 + 갭 17항목 추가**(A 기능7·C UX6·B 신뢰7, 우선순위 사용자 확정 A→C→B). 갭은 QA 실측 역산으로 도출. core_factor는 **자동 생성 기본**(사람 확정은 WS8 상품 사용 시만 — 사용자 확정). FE-1 지시서 확정판 재작성, FE 레인 가동.
- 2026-07-07 (밤5): **F1 클러스터 잔재 — 공식 종결. legacy(run_tag null) 0행 달성** (시작 1,992행 → 0). 마지막 3케이스는 신코드 재실행으로 해소(6769b0bb v20·f56fee3f v15·c31e5a68 빈결과 정직 청소). 전 케이스 serve-stats 동반(R11). + ORCH 인수인계 문서 작성(docs/ORCH_인수인계.md — 검증 프로토콜·판단 기준·치트시트).
- 2026-07-07 (밤4): **BE 루프 6건 머지·배포 + migration 020/021 프로덕션 적용** — 죽은 테이블 6개 drop(가드 통과)·status 통일·RLS 전 테이블·channel NOT NULL(빈 케이스 3건 'other' 백필). BE-5 근본원인 정정: step 상한 초과는 pass1이 아니라 **read-key-stats가 key_stats 6.9MB를 통째 반환**한 것 — 8개 phase 슬림화. BE-9 판정: Q7 핸들 0건은 파서 정상(UTM에 핸들 없음), **creator_page_name(43/221)을 쓰라**는 대안 제시. BE-3 실측이 spec/01 '전부 0행' 반증(sales_monthly 650 등) → spec 정정.
- 2026-07-07 (밤3): **질문 계약 v2 채택** (CX-2 리뷰, 사용자 위임) — Q0 채택게이트·Q6 core_factor·Q7 top-N·Q8 freshness 신설, v1-Q5 렌즈 강등, v1-Q6·7 통합. + **D4 확정: 원천 인입은 수동 유지**(API 구독 비용 사유), 자동화는 설계만 백로그 보존(BE-4).
- 2026-07-07 (밤2): **게이트 v2(재호스트 표본 30+30) 최종 판정 — 모델 티어링 전면 보류, Sonnet 유지.** 핵심 발견: ①Sonnet 자기일치가 영상 56%(cta 27%)에 불과 — 태깅 어휘 모호가 근본 문제(Q3·Q5 신뢰 상한, 사용자의 "비전 태깅 안 됨" 체감의 정량 근거) ②Haiku는 핵심 해석 필드에서 베이스라인 대비 붕괴(영상 앵글 60→43%·광고 origin_class 80→50%). 순서 재정의: BE-6 어휘 조이기 → 자기일치 ≥85% → Haiku 재게이트.
- 2026-07-07 (밤): **WS3 게이트 실행 → Haiku 전환 보류 판정** — 광고 scalar 85%(<90%, origin_class 70%·hook_type 60%가 미달 원인), 영상은 커버 URL 만료로 52/60 비교 불능(R9 — 게이트 스크립트가 재호스트 이미지 써야 함, 수정 후 재도전). **WS3는 Sonnet 기본값으로 머지·배포** — dedup(재과금 방지)·광고 태깅 확장(source_channel·banner_style)·v_case_ad_runtime은 모델 무관 가치라 즉시 활성. 티어링 절감(~$11/케이스)은 게이트 재통과 시 env 전환.
- 2026-07-07 (저녁): WS5 파이프라인 수리 3건 배포 (오케스트레이터 직접 실행, 각 1fix=1배포=1검증) — ①운영 가드 3종(실패 슬랙 알림·credit_exhausted 분류·케이스 $25/월간 $300 비용 상한, shared.ts, 배포 후 collect-yt 캐시히트로 가드 경로 검증) ②조용한 손실 2건 fix(uploadBsr delete→upsert 순서 역전으로 손실 창 제거 + mergeCases phase_runs 이동 누락, deleteCase는 검증 결과 안전 확인) ③F2 조건부 enrich(TT·IG 공통 — 대형 풀 300+ 시 영상/포스트 3+만 조회, env 조정 가능). 잔여: validated-0(§2)는 WS3 머지 후, 구조 청소(§4)·E2E(§7)는 WS5 실행 세션 몫.
- 2026-07-07 (오후): **WS4 2분할 (사용자 결정 — "화면 기획 먼저 보고 시작")** — WS4a=화면 목업(`/mockup/*`, 실케이스 fixture, U1~U8 시연, 검수 체크리스트+결정 요청 목록)을 사용자가 화면으로 검수·확정한 뒤 WS4b=교차 뷰+데이터 결선. WS3은 병렬. 발사 순서 시작_가이드 갱신.
- 2026-07-07: **WS3 코드 완료** (브랜치 ws-3-model-tiering) — ①모델 티어링: 영상·광고 태깅+클러스터 pass1 → Haiku 4.5(`pricing.ts`, `BP_TAGGING_MODEL` override), pass2/3·SKU는 Sonnet 유지, 클러스터 비용 pass별 단가 분리. ②dedup(`dedup.ts`): 태깅 입력 해시(cover/thumbnail+caption+asr)로 "다른 행·같은 입력" 재태깅 방지(케이스 무관 재사용+배치 내 dedup), `tag_input_hash` 컬럼 2개(migration 018)+reused 카운트. ③광고 태깅 확장(Q6): ad_intel에 source_channel·banner_style 추가(신규 필드 null 허용), `v_case_ad_runtime` 최장운영 랭킹 뷰(018). ④품질 게이트 `scripts/gate-tagging-model.ts`(Sonnet vs Haiku 30샘플 일치율) — **ANTHROPIC_API_KEY 비어 미실행**(크레딧 대기), 스크립트 완성·검증. tsc 통과. 마이그레이션 적용·배포·백필·게이트 실호출은 오케스트레이터 몫. 상세 `docs/ws/WS3_REPORT.md`.
